...
public class ZassenhausTest {
  ...
  private static final Tuple<MultiVarPolynomial> C2 = lieBracket(X, Y).multiply(new Rational(1, 2));
  private static final Tuple<MultiVarPolynomial> C3 =
      (lieBracket(Y, lieBracket(X, Y)).multiply(new Rational(1, 3)))
          .add(lieBracket(X, lieBracket(X, Y)).multiply(new Rational(1, 6)));
  private static final Tuple<MultiVarPolynomial> C4 =
      (lieBracket(Y, lieBracket(Y, lieBracket(X, Y)))
              ...
              .multiply(new Rational(1, 8)))
          .add(lieBracket(X, lieBracket(X, lieBracket(X, Y))).multiply(new Rational(1, 24)));
