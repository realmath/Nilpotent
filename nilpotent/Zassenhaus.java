...
class Zassenhaus {
  ...
  Tuple<MultiVarPolynomial> get(int n) {
    ...
    if (n <= 4) {
      retVal = ff(1, n - 1).multiply(Rational.Companion.valueOf(n).reciprocal());
    } else {
      retVal = ff((n - 1) / 2, n - 1).multiply(Rational.Companion.valueOf(n).reciprocal());
    }
  ...
  private Tuple<MultiVarPolynomial> f1k(int k) {
    Tuple<MultiVarPolynomial> retVal = Polynomials.INSTANCE.zeroTuple(dim);
    for (int j = 1; j <= k; j++) {
      Rational coef = Rational.Companion.factorial(j).multiply(Rational.Companion.factorial(k - j)).reciprocal();
      if (k % 2 != 0) {
        coef = coef.negate();
      }
      retVal =
          retVal.add(adjointYPow(adjointXPow(Polynomials.INSTANCE.basePols2(dim), j), k - j).multiply(coef));
    }
    return retVal;
  }
  ...
  private Tuple<MultiVarPolynomial> fnk(int n, int k) {
    ...
    for (int j = 0; j <= k / n - 1; j++) {
      Rational coef = Rational.Companion.factorial(j).reciprocal();
      if (j % 2 != 0) {
        coef = coef.negate();
      }
  ...
  private Tuple<MultiVarPolynomial> adjointXPow(Tuple<MultiVarPolynomial> v, int k) {
    return adjointPow(Polynomials.INSTANCE.basePols(dim), v, k);
  }

  private Tuple<MultiVarPolynomial> adjointYPow(Tuple<MultiVarPolynomial> v, int k) {
    return adjointPow(Polynomials.INSTANCE.basePols2(dim), v, k);
  }
